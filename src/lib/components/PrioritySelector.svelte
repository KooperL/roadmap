...
<Button className="whitespace-nowrap"
	>{selectedPriority ? getPriorityName(selectedPriority) : 'Priority'}
	<AngleDownOutline class="me-2 h-4 w-4" /></Button
>
<Select
	simple
	items={Object.keys(cardPriority).map((key) => ({
		component: {
			component: Chip,
			props: { color: getPriorityColor(cardPriority[key]) },
			text: key
		},
