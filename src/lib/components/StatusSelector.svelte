...
{#if $projectStatusState.status === fetchStatus.loading}{:else if $projectStatusState.status === fetchStatus.success}
	<Button className="whitespace-nowrap"
		>{selectedStatus ? selectedStatus : 'Status'}<AngleDownOutline class="me-2 h-4 w-4" /></Button
	>
	<Select
		isOpen
		simple
		items={$projectStatusState.data.map((statusObject) => ({
