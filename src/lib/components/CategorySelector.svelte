...
<Button className="whitespace-nowrap">{selectedCategory ? getCategoryName(selectedCategory) : 'Category'}<AngleDownOutline class="me-2 h-4 w-4" /></Button>
<Select isOpen={isOpen} simple items={[
  ...$cardCategoryState.data.map(category => ({
      text: category.name,
      action: () => {selectCategory(category.id)}
  })),
  {
